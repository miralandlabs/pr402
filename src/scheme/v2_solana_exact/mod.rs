...
impl X402SchemeFacilitator for V2SolanaExactFacilitator {
    ...
    {
        crate::seller_provision::build_universalsettle_seller_provision(
        ...
        .await
        .map_err(|e| match e {
            crate::seller_provision::SellerProvisionError::InvalidInput(m)
            | crate::seller_provision::SellerProvisionError::MintNotAllowed(m) => {
                X402SchemeFacilitatorError::InvalidPayload(m)
            }
            crate::seller_provision::SellerProvisionError::Chain(m) => {
                X402SchemeFacilitatorError::OnchainFailure(m)
            }
        })
    }
