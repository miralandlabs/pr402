...
- `POST /api/v1/facilitator/build-sla-escrow-payment-tx` — Build an **unsigned** SLA-Escrow **`FundPayment`** transaction (compute budget + optional escrow vault ATA create + fund). Body includes `payer`, `accepted` (`scheme: "sla-escrow"`), `resource`, **`slaHash`** (64 hex), **`oracleAuthority`**, optional `paymentUid`, optional `skipSourceBalanceCheck`. The **buyer** is both **fee payer** and **signer** (facilitator must not appear in instruction accounts). Requires `ESCROW_PROGRAM_ID` / SLA escrow config. Classic SPL Token mints only in this builder (Token-2022 and native SOL fund layouts: use `sla-escrow` CLI or extend the builder).
