...
function handle_paid_request(request):
    ...
    # Do not deliver here. If continuing, call /settle immediately with the same proof.
...
try:
    ...
    payment_body = json.loads(proof)  # compatibility with raw-JSON clients
...
try {
  ...
  paymentBody = JSON.parse(proof); // compatibility with raw-JSON clients
}
...
if err != nil || !json.Valid(paymentBody) {
    paymentBody = []byte(proof) // compatibility with raw-JSON clients
}
